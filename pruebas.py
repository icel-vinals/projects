...
def mult_vec_escalar(v1,esc,largo):
    Vector = [0,0,0]
    for i in range(largo):
        Vector[i] = v1[i]*esc
    return Vector
...
def dibujar_linea(punto1, punto2, color):
    if punto1[0] < 1 or punto2[0] < 1:
        if punto1[0] < 1 and punto2[0] < 1:
            return 0
        if punto1[0] < 0:
            prop = punto2[0]/(punto2[0]-punto1[0])*.99
            vec = mult_vec_escalar(resta_vec(punto1,punto2,3),prop,3)
            punto1 = suma_vec(punto2, vec,3)
            if punto1[0] == 0:
                punto1[0] = .0005

        if punto2[0] < 0:
            prop = punto1[0] / (punto1[0] - punto2[0])*.99
            vec = mult_vec_escalar(resta_vec(punto2, punto1, 3), prop, 3)
            punto2 = suma_vec(punto1, vec, 3)


    print(punto1[0],punto2[0])
